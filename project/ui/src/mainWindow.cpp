...
: QWidget(parent)
{
    ...
    QStringList list;
    list << "1 этаж" << "2 этаж" << "3 этаж" << "4 этаж";
    list << "5 этаж" << "6 этаж" << "7 этаж" << "8 этаж" << "9 этаж";
    ...
    connect(lvlFloor->selectionModel(),
                                     ...
                                     this,SLOT(onLvFloorChanged(const QModelIndex, const QModelIndex)));
...
void MainWindow::mouseDoubleClickEvent(QMouseEvent *event) {
    qDebug() << "mouse double clecked - " << event->pos().x() << " " << event->pos().y();
}

void MainWindow::wheelEvent(QWheelEvent *event) {
    qDebug() << " wheelEvent " << event->angleDelta().y();
    qreal scale = controller->drawer->getScale();
    qDebug() << "  current scale - "  << scale;
    scale += event->angleDelta().y() > 0 ? 0.01 : -0.01;
