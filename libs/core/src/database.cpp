...
void Database::insert_json(const std::string &json_str) {
    std::string comand = "INSERT INTO " + table_name + "(id, doc) VALUES (1, '" +
                         json_str + "') " + "ON CONFLICT (id) DO UPDATE SET " +
                         "doc= '" + json_str + "' where " + table_name + ".id=1;";
...
std::string Database::read_json() {
    std::string comand = "SELECT doc from " + table_name;
