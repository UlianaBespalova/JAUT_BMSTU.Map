...
void Drawer::drawImage(Point center, const std::string &path) {
  QImage image(QString::fromStdString(path));
  if (!image.isNull()) {
    Point pos;
    pos.x = center.x - image.size().width() / 2;
    pos.y = center.y - image.size().height() / 2;
    image = image.scaled(image.size().width() * scale,
                         image.size().height() * scale);
    painter->drawImage(rePoint(pos), image);
  } else {
...
Point Drawer::getViewPos() { return view_pos; }
